...
if(isset($_GET['page']) && $_GET['page']!='')
{
	switch($_GET['page'])
	{
		...
		case 'afficher':
			...
			mysql_connect($server,$user,$pass);
			$result=mysql_db_query($base,"SELECT `id_client`, `etat_commande`,`date_commande`,`heure_commande` FROM `commande_liste_index` WHERE `id_commande`='".$_GET['id_commande']."'");
...
<?
				...
				$result_client=mysql_db_query($base,"SELECT * FROM `base_client` WHERE `id_client`='".$ra['id_client']."'");
				$ra_client=mysql_fetch_array($result_client);
				//echo mysql_error()."SELECT * FROM `base_client` WHERE `id_client`='".$ra['id_client']."'";

				$result_zone=mysql_db_query($base,"SELECT `titre_zone` FROM `zone_livraison` WHERE `id_zone`='".$ra_client['id_zone']."'");
				$ra_zone=mysql_fetch_array($result_zone);
...
<?
			...
			echo '<div class="commande_ticket_titre"><center>INFORMATION CLIENT</center></div>';
			echo '<div class="commande_ticket_info">'.htmlentities($ra_client['nom_client']).' '.htmlentities($ra_client['prenom_client']).' ( '.htmlentities($ra_client['tel_client']).' )<br /><br /> '.htmlentities($ra_client['adresse_client']).'<br /> '.$ra_zone['titre_zone'].'<br /><br /> '.$ra_client['digicode_client'].'</div>';
			echo '<div class="commande_ticket_titre"><center>A LA CARTE</center></div>';
